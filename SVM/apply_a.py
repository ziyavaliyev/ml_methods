...
train.update({'data': np.loadtxt('/Users/ziya03/Downloads/solution/q1_svm_python/lc_train_data.dat')})
train.update({'label': np.loadtxt('/Users/ziya03/Downloads/solution/q1_svm_python/lc_train_label.dat')})
test.update({'data': np.loadtxt('/Users/ziya03/Downloads/solution/q1_svm_python/lc_test_data.dat')})
test.update({'label': np.loadtxt('/Users/ziya03/Downloads/solution/q1_svm_python/lc_test_label.dat')})
...
# Train the SVM with the written SVM function
print('Training the SVM')
alpha, sv, w, b, result, slack = svmlin(train['data'], train['label'], C)
print('Number of SV: {0}\n'.format(sum(sv)))

# Train the SVM with scikit-Learn
print('Training with scikit-learn')
clf = svm.LinearSVC()
clf.fit(train['data'], train['label'])
result2 = clf.predict(test['data'])

# Accuracy on train data
accuracy1 = len(result[np.sign(result) == train['label']])/len(train['label'])
accuracy2 = len(result2[np.sign(result2) == test['label']])/len(test['label'])
print('Accuracy on train data with C = {0}: \t {1}'.format(C, accuracy1))
print(f'Accuracy of test data with the scikit-learn training: {accuracy2}')
